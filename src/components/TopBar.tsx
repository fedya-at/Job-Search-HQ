...
import { SheetTab, Application, ApplicationStatus } from '../types';
...
export const TopBar: React.FC<TopBarProps> = ({
  ...
  return (
    <header className="sticky top-0 z-20 bg-white/95 backdrop-blur-md border-b border-gray-100 px-4 sm:px-6 lg:px-8 py-3.5">
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
        {/* Left: Mobile Menu Toggle & Title */}
        <div className="flex items-center gap-3 min-w-0">
          <button
            onClick={onOpenMobileMenu}
            className="md:hidden p-2 rounded-xl text-gray-600 hover:text-[#003049] hover:bg-gray-100 cursor-pointer"
            title="Open Menu"
          >
          ...
          <div className="min-w-0">
            <h2 className="text-lg sm:text-xl font-bold text-[#003049] truncate">
              {activeTabMeta.title}
            </h2>
        ...
        <div className="flex items-center gap-2.5 max-w-lg w-full justify-end">
          ...
          <div className="relative flex-1 max-w-md" ref={searchContainerRef}>
            <div className="relative flex items-center">
              ...
              <input
                ...
                className="w-full pl-9 pr-14 py-2 text-xs rounded-xl bg-gray-50 border border-gray-200 text-[#003049] placeholder-gray-400 focus:outline-none focus:border-[#780000] focus:ring-1 focus:ring-[#780000]/10 transition-all shadow-2xs"
              />
              <div className="absolute right-2.5 flex items-center gap-1">
                {searchQuery ? (
                  <button
                    ...
                    className="p-0.5 text-gray-400 hover:text-[#780000] rounded-lg"
                    title="Clear search"
                  >
                    <X className="w-3.5 h-3.5" />
                  </button>
                ) : (
                  <span className="hidden sm:inline-flex items-center px-1.5 py-0.5 text-[10px] font-mono text-gray-400 bg-white border border-gray-200 rounded">
                    /
                  </span>
                )}
              </div>
            ...
            {isSearchFocused && searchQuery.trim().length > 0 && (
              ...
              >
                <div className="px-4 py-2 bg-gray-50 border-b border-gray-100 flex items-center justify-between text-xs text-[#003049]">
                  <span className="font-medium text-[11px]">
                    Matches for <strong className="text-[#780000]">"{searchQuery}"</strong> ({searchMatches.length})
                  </span>
                  <span className="text-[10px] text-gray-400">Press Enter for table</span>
                </div>
                    ...
                    {searchMatches.slice(0, 6).map((app) => {
                      ...
                      return (
                        <div
                          ...
                          className="px-4 py-2.5 hover:bg-gray-50 cursor-pointer transition-colors flex items-center justify-between gap-3 group text-xs"
                        >
                          <div className="min-w-0">
                            <div className="font-semibold text-[#003049] group-hover:text-[#780000] truncate">
                              {app.company} <span className="font-normal text-gray-500">— {app.jobTitle}</span>
                            </div>
                            {app.contactName && (
                          ...
                          <span
                            className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium border shrink-0 ${statusConfig.bg} ${statusConfig.text} ${statusConfig.border}`}
                          >
          ...
          <button
            ...
            className="inline-flex md:hidden items-center gap-1 px-3 py-2 rounded-xl bg-[#780000] hover:bg-[#c1121f] text-white text-xs font-semibold shadow-xs transition-colors shrink-0"
          >
